...
async def main():
    ...
    if sys.platform == "emscripten":
        try:
            import platform
            browser_document = platform.window.document
            browser_name_input = browser_document.createElement("input")
            browser_name_input.id = "overtime-name-input"
            browser_name_input.type = "text"
            browser_name_input.maxLength = 12
            browser_name_input.autocomplete = "off"
            browser_name_input.autocorrect = "off"
            browser_name_input.autocapitalize = "characters"
            browser_name_input.spellcheck = False
            browser_name_input.inputMode = "text"
            browser_name_input.setAttribute("inputmode", "text")
            browser_name_input.setAttribute("enterkeyhint", "done")
            browser_name_input.readOnly = False

            style = browser_name_input.style
            ...
            # Safari/iPad deve receber o toque diretamente no input HTML.
            # Assim o proprio elemento abre o teclado virtual, sem depender
            # de pygame.MOUSEBUTTONDOWN/FINGERDOWN.
            def _focar_input(event=None):
                try:
            ...
            if browser_canvas is None:
                browser_canvas = browser_document.querySelector("canvas")
        except Exception as e:
            print(f"Aviso: nao foi possivel criar o campo HTML do Safari: {e}")
            browser_name_input = None
            browser_document = None
            browser_canvas = None

    def atualizar_campo_nome_browser(visible):
        if browser_name_input is None:
            return

        try:
            canvas = browser_canvas
            if canvas is None and browser_document is not None:
                canvas = browser_document.querySelector("canvas")

            if canvas is None:
                return

            rect = canvas.getBoundingClientRect()
            escala_x = float(rect.width) / float(LARGURA)
            escala_y = float(rect.height) / float(ALTURA)

            browser_name_input.style.left = f"{float(rect.left) + box_nome_rect.x * escala_x}px"
            browser_name_input.style.top = f"{float(rect.top) + box_nome_rect.y * escala_y}px"
            browser_name_input.style.width = f"{box_nome_rect.width * escala_x}px"
            browser_name_input.style.height = f"{box_nome_rect.height * escala_y}px"
            browser_name_input.style.fontSize = f"{max(14, int(18 * escala_y))}px"
            browser_name_input.style.display = "block" if visible else "none"
        except Exception:
            pass
    ...
    while rodando:
        ...
        for evento in pygame.event.get():
            ...
            if estado_atual == ESTADO_NOME:
                if evento.type == pygame.KEYDOWN and browser_name_input is None:
                    ...
                    if box_nome_rect.collidepoint(pos_x, pos_y):
                        ...
                        if browser_name_input is not None:
                            try:
                                browser_name_input.style.display = "block"
                                browser_name_input.focus()
                            except Exception:
                                pass
        ...
        if browser_name_input is not None and estado_atual != ESTADO_NOME:
            try:
                browser_name_input.style.display = "none"
            except Exception:
                pass
